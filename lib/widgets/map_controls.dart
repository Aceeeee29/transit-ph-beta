...
class _MapControlsState extends State<MapControls> {
  ...
  Widget build(BuildContext context) {
    ...
    final totalDistance = RouteMetricsService.calculateRouteDistance(widget.pathPoints);
    ...
    final eta = RouteMetricsService.calculateEta(widget.pathPoints, modes, widget.stepBoundaries);
    final formattedEta = RouteMetricsService.formatEta(eta);
    
    final fare = RouteMetricsService.calculateFareEstimate(widget.pathPoints, modes, widget.stepBoundaries);
            ...
            if (widget.pathPoints.isNotEmpty) ...[
              Row(
                ...
                children: [
                  ...
                  IconButton(
                    icon: Icon(_showMetrics ? Icons.visibility : Icons.visibility_off),
                    onPressed: () {
                      setState(() {
            ...
            Wrap(
              ...
              children: [
                ...
                SizedBox(
                  width: 40,
                  child: IconButton(
                    onPressed: widget.historyService.canUndo ? widget.onUndo : null,
                    icon: const Icon(Icons.undo, size: 18),
                    tooltip: 'Undo',
                    color: widget.historyService.canUndo ? Colors.blue : Colors.grey,
                ...
                SizedBox(
                  width: 40,
                  child: IconButton(
                    onPressed: widget.historyService.canRedo ? widget.onRedo : null,
                    icon: const Icon(Icons.redo, size: 18),
                    tooltip: 'Redo',
                    color: widget.historyService.canRedo ? Colors.blue : Colors.grey,
                    padding: const EdgeInsets.all(4),
                    constraints: const BoxConstraints(),
                  ),
                ),
                ...
                SizedBox(
                  width: 40,
                  child: IconButton(
                    onPressed: widget.pathPoints.isNotEmpty ? widget.onReset : null,
                    icon: const Icon(Icons.refresh, size: 18),
                    tooltip: 'Reset',
                    color: widget.pathPoints.isNotEmpty ? Colors.red : Colors.grey,
                    padding: const EdgeInsets.all(4),
                    constraints: const BoxConstraints(),
                  ),
                ),
...
class RoutePreviewOverlay extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Positioned(
      ...
      .slideY(begin: 0.5, end: 0, duration: 300.ms, curve: Curves.easeOutQuad);
  }
