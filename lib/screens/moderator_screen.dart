...
class _ModeratorScreenState extends State<ModeratorScreen>
  ...
  Widget _buildFeedbacksTab() {
    final feedbacks =
        ModerationService.getFeedbacks() as List<feedback_model.Feedback>;
    return ListView.builder(
      itemCount: feedbacks.length,
      itemBuilder: (context, index) {
        final feedback = feedbacks[index] as feedback_model.Feedback;
        if (feedback.type == feedback_model.FeedbackType.report) {
          final post = ModerationService.getPost(feedback.targetId!);
          if (post != null) {
            return Card(
              margin: const EdgeInsets.all(8),
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(post.content),
                    const SizedBox(height: 8),
                    Text('Reported by: ${feedback.userId}'),
                    const SizedBox(height: 8),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.end,
                      children: [
                        ElevatedButton(
                          ...
                          child: const Text('Reject Post'),
                        ),
                        const SizedBox(width: 8),
                        ElevatedButton(
                          onPressed: () {
                            // Dismiss report, remove feedback
                            ModerationService.feedbacks.remove(feedback);
                            setState(() {});
                          },
                          style: ElevatedButton.styleFrom(
                            backgroundColor: Colors.green,
            ...
            return Card(
              ...
              child: Padding(
                padding: const EdgeInsets.all(16),
                child: Text('Post not found for report: ${feedback.targetId}'),
              ),
            );
          }
        } else {
